"""
Drive-momentum backtest — FLIP bracket only.
...
Default bracket (1 mini):
  target 8.5 pts  |  stop 50 pts  |  baseline ~85.5%

Pass --legacy-bracket for the original 7.5 / 50 research bracket from backtest.py.
...
# 1 mini: 8.5 pt target / 50 pt stop (~$170 / $1k at $20/pt)
FLIP_LIVE = Bracket("flip", 8.5, 50.0)
FLIP_LEGACY = Bracket("flip_legacy", 7.5, 50.0)


def main() -> None:
    ap = argparse.ArgumentParser(description="TopHat drive backtest — FLIP")
    ap.add_argument("--samples-per-day", type=int, default=40,
                    help="coin-flip control samples per day")
    ap.add_argument("--seed", type=int, default=1)
    ap.add_argument("--slip", type=float, default=0.0,
                    help="adverse entry slippage in points")
    ap.add_argument("--monthly", action="store_true",
                    help="print drive win rate by month")
    ap.add_argument("--legacy-bracket", action="store_true",
                    help="use 7.5/50 bracket from original backtest.py")
    ap.add_argument("--cache", default=CACHE_FILE)
    args = ap.parse_args()

    br = FLIP_LEGACY if args.legacy_bracket else FLIP_LIVE
